...
export const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const focusableEls = mobileMenuRef.current.querySelectorAll<HTMLElement>(focusableSelectors.join(','));
    ...
    >
      ...
      {isMobileMenuOpen && (
        <div
          ...
          aria-modal="true"
          role="dialog"
          className="lg:hidden absolute top-full left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-lg p-4 flex flex-col gap-4"
        >
          {navItems.map((item) => (
            <a
              key={item.label}
